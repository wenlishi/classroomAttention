...
def create_class_session(db: Session, session: session_schema.SessionCreate, owner_id: int )->models.ClassSession:
    """在数据库中创建一条新的课堂会话记录"""
    session_data = session.model_dump()
    
    db_session = models.ClassSession(**session_data, owner_id=owner_id)
    db.add(db_session)
    db.add(db_session)
    db.commit()
